...
def preprocess_image(image_path):
    ...
    preprocessed_image_path = "E:\\Mini_Project\\preprocessed_image.png"  #Path to save processed image
...
def extract_text_from_image(preprocessed_image_path):
    try:
        ocr = PaddleOCR(lang='en')  # Initialize PaddleOCR with English language
        results = ocr.ocr(preprocessed_image_path)    # Perform OCR and angle correction
...
def extract_text_from_pdf(pdf_path):
    ...
    for page_num in range(len(pdfimg)): #Loops through each page
        ...
        saveto = "E:\\Mini_Project\\Bills\\pdf_images\\"  #Directory to save images
...
def handle_attachment(part):
    ...
    if filename:
        ...
        download = os.path.join("E:\\Mini_Project\\Bills\\",filename) #Saves file at given path
        os.makedirs("E:\\Mini_Project\\Bills\\", exist_ok = True) #Creates path if does not exists
...
def send_mail(excel_file, user, password):
    ...
    msg['to'] = "#####@gmail.com"  #Send mail to ID
...
if __name__ == "__main__":
    email_user = "#####@gmail.com" #Add Gmail account
    email_pass = "#### #### #### ####"  #add your app password
